...
from sanity_check_src.utils import TrajectoryDataset
...
result_filename = 'loader_val.p'
with open(join(dataset_folderpath, result_filename), 'wb') as f:
    pickle.dump(loader_val, f)
...
result_filename = 'loader_train.p'
with open(join(dataset_folderpath, result_filename), 'wb') as f:
    pickle.dump(loader_train, f)
    print(result_filename+' is created.')
...
batch = get_batch_sample(loader_train)
obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel, non_linear_ped,\
    loss_mask,V_obs,A_obs,V_tr,A_tr = batch
